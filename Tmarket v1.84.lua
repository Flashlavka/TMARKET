    ...
    checkNick(cachedNick, function(hasAccess)
        if hasAccess then
            downloadConfigFile(function()
                loadData()
                sampAddChatMessage(string.format("{A47AFF}[Tmarket]{FFFFFF} çàãðóæåí  |  Àêòèâàöèÿ: {A47AFF}/tmarket{FFFFFF}  |  Âåðñèÿ: {A47AFF}v%s{FFFFFF}  |  Àâòîð: {FFD700}legacy.", thisScript().version), -1)
                sampRegisterChatCommand("tmarket", function() window[0] = not window[0] end)
            end)
        else
            sampAddChatMessage("{FF8C00}[Tmarket] {FFFFFF}Ó âàñ {FF0000}íåò äîñòóïà{FFFFFF}.", -1)
        end
    end)
...
if not imgui.Begin(u8("Tmarket — Òàáëèöà öåí v" .. thisScript().version .. ". Àâòîð — legacy."), window) then imgui.End() return end
        ...
        if #filtered > 0 then
            ...
            local text = u8("Òîâàðû íå íàéäåíû")

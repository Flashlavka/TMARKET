...
function main()
    ...
    if checkNick(cachedNick) then
        ...
        sampAddChatMessage("{4169E1}[Tmarket çàãðóæåí v1]{FFFFFF}. {00BFFF}Àêòèâàöèÿ:{FFFFFF} {DA70D6}/lm {FFFFFF}. Àâòîð: {1E90FF}legacy{FFFFFF}", 0x00FF00FF)
    else
        ...
        sampAddChatMessage("{FF8C00}[Tmarket] {FFFFFF}Âû ìîæåòå ïðèîáðåñòè ñêðèïò - {1E90FF}https://t.me/legacy_chay{FFFFFF}", 0xFFFFFF)
        return
    end
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.Begin(u8("Tmarket - Îíëàéí Òàáëèöà Öåí â Ñêðèïòå"), window)

        imgui.InputTextWithHint("##search", u8("Ïîèñê ïî òîâàðàì..."), search, ffi.sizeof(search))
        imgui.SameLine()
        if imgui.Button(u8("Â ðàçðàáîòêå")) then saveData() end

        imgui.Separator()
        imgui.Columns(3)
        imgui.Text(u8("Òîâàð")); imgui.NextColumn()
        imgui.Text(u8("Ñêóïêà")); imgui.NextColumn()
        imgui.Text(u8("Ïðîäàæà")); imgui.NextColumn()
        imgui.Separator()
